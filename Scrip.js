...
FormularioBorrar.addEventListener('submit',function(){
    var MateriaBorrar = document.getElementById("materia").value;
    if(MateriaBorrar.localStorage!=null){
        localStorage.removeItem(MateriaBorrar)
    }
})
*/
...
function consultarMaterias() {
    ...
    materias.forEach(function(item,index,array) {
        ...
        `<tr> 
            ...
            <td align="center" style="dislay: none;"> ${item.Nota3 }</td> 
            <td align="center" style="dislay: none;"> ${item.Promedio }</td> 
            <td align="center" style="dislay: none;"> <button ounclick= "Eliminar(${index})"> Eliminar</button> </td>
                </tr>`;
    });
   }

   
   function Eliminar(indiz){
       let materias = JSON.parse(localStorage.getItem("BDLocal"));
       materias.splice(indiz,1);
       localStorage.setItem("BDLocal",JSON.stringify(materias));
       location.reload();
       consultarMaterias();
